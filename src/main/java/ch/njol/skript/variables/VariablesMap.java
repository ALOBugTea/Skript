...
final class VariablesMap {
    ...
    Object getVariable(final String name) {
		if (!name.endsWith("*")) {
			...
			for (int i = 0; i < split.length; i++) {
				...
				final Object childNode = parent.get(n);
				if (childNode == null)
					return null;
		...
		for (int i = 0; i < split.length; i++) {
			...
			if (childNode == null) {
				...
				if (i == split.length - 1) {
					...
					childNode = new TreeMap<>(variableNameComparator);
				...
				if (i == split.length - 1) {
					...
					TreeMap<String, Object> newChildNodeMap = new TreeMap<>(variableNameComparator);
	...
	private static TreeMap<String, Object> copyTreeMap(TreeMap<String, Object> original) {
		TreeMap<String, Object> copy = new TreeMap<>(variableNameComparator);
