...
public abstract class VariablesStorage implements Closeable {
			...
			public void run() {
				while (!closed) {
					try {
						...
						final SerializedVariable variable = changesQueue.take();
						final Value value = variable.value;
	 ...
	 */
	@Nullable
	protected String getValue(final SectionNode sectionNode, final String key) {
	...
	public final boolean load(final SectionNode n) {
		...
		if (requiresFile()) {
			// Initialize file
			final String fileName = getValue(n, "file");
			if (fileName == null)
				return false;

			final File file = getFile(fileName).getAbsoluteFile();
			this.file = file;
	...
	public void startBackupTask(final Timespan backupInterval) {
	 ...
	 * and from the {@link #writeThread} afterward.
	 * <p>
